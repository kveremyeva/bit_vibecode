...
export function resolvePeriod(period, from, to, now = new Date()) {
  ...
  if (key === "custom") {
    ...
    // M4: значение из <input type=date> приходит как "YYYY-MM-DD" и парсится
    // как полночь; фильтр дат включительный, поэтому конец «до» сдвигаем
    // на последнюю миллисекунду этого же дня, а не на полночь.
    if (typeof to === "string" && /^\d{4}-\d{2}-\d{2}$/.test(to)) {
      end = Date.UTC(...to.split("-").map(Number)) + 86_400_000 - 1;
    }
